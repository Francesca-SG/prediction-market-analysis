# Kalshi Scrape and Analysis

### Scraper Description
The scraper is a python script that pulls data directly from Kalshi's API, collecting the data in layers (series, markets, events). I limited the scraper to a single category, to suit the small scope of this project. To avoid flooding the API with too many requests, I added a short delay after collecting each set of data. Progress is saved on every loop, ensuring data is not lost if the script breaks. Once the script has finished running, the results are assembled with pandas DataFrame and written to CSV, ready for the analysis phase.

The dataset collected for this project only focuses on a single category, however the same approach could be extended to all categories if a broader analysis was required.
...
The event "Which party will win the U.S. House of Representatives?" has over $5 million in total market volume and more then $3 million in open interest, making this a highly active event across the dataset. The average Yes and No bids sit at exactly 50 cents, reflecting deep engagement but real uncertainty amongst traders.
